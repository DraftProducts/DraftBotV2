...
class MusicPlayer {
	...
	playSong(msg) {
		if (this.queue.length === 0) {
			this.musicChannel.send(
				':musical_note: | Musiques en attente terminés.'
			);
		...
		function resolveVoiceChannel() {
			return new Promise((resolve, reject) => {
				if (this.voiceConnection) resolve();
				else {
					msg.channel.send(
						`:no_entry_sign: | Veuillez inviter le bot dans votre salon à l'aide du ` /
							join` avant de lancer une musique.`
					);
					reject('novoice');
				}
			});
	...
	setVolume(msg) {
		...
		if (vol && (vol >= 0 && vol <= 1)) {
			if (this.dispatch) {
				...
				this.volume = vol;
				msg.channel.send(
					`:speaker: | Le volume viens d'être réglé sur ${tool.wrap(
						vol * 100
					)}`
				);
			} else {
	...
	joinVc(msg) {
		if (msg.member.voiceChannel) {
			...
			msg.channel.send(
				":no_entry_sign: | Vous n'êtes pas dans un solon vocal !"
			);
		}
	}
	leaveVc(msg) {
